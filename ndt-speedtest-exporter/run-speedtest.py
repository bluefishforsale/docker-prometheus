...
parser.add_argument("-p", "--port", type=int, default=9140,
...
gauge_speedtest = Gauge('ndt_speedtest', 'M-Labls Speedtest', ['name', 'server', 'metro'])

def process_request(t):
  command = f'/go/bin/ndt7-client -quiet -format=json'.split(' ')
  _results = run(command, stdout=PIPE, stderr=PIPE, universal_newlines=True)
  _dict = json.loads(_results.stdout)
